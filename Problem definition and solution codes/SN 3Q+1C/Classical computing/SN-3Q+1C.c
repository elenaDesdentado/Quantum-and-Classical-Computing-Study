...
// Bernstein-Vazirani (Secret number) - 3 positions (~qubits)

int secret_number[3] = {0,1,0};
...
void main(){
    ...
    for(int j = 0; j < 9, found == false; ++j) {
        // Binary Counter
        try_number[i] = j/4%2; try_number[i+1] = j/2%2; try_number[i+2] = j%2;

        obtained_response = oracle(try_number, size_of_try_number);

        if (obtained_response == 'O' &&  (get_number_of_ones(try_number, size_of_try_number)== 1)){
            found = true;
        }
    }
